...
`ChinaMax` 是 blackmatrix7 维护的中国大陆常用域名、IP 和服务规则大合集，本配置把它命名为 `国内网站合集`，放在末尾作为国内直连兜底。
...
可选 Loyalsoldier Direct（已由 Actions 转换为 QuanX 格式，规则量较大，默认关闭）：
...
| `Rules/Generated/Loyalsoldier/Direct.list` | 自动生成的 Quantumult X 规则。 |
...
| `tools/generate-loyalsoldier-rules.js` | 维护脚本。 |
| `tools/generate-loyalsoldier-rules.test.js` | 维护脚本。 |
